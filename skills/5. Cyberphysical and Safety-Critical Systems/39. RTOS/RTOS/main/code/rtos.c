...
// uint32_t twenty_readings[20];
// uint32_t average = 0;
// uint32_t prev_average = 0;
...
static void compare_task(){
  ...
  printf("Average is, %d \n", average);
  if (average > prev_average) led_status = 1; // pos Red
  else if (average == prev_average) led_status = 0; // no change Green
  else if (average < prev_average) led_status = -1;
}
*/
void led_task(){
